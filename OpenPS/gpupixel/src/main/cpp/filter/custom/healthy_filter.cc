...
const std::string kHealthyFragmentShaderString = SHADER_STRING(
...
std::shared_ptr<HealthyFilter> gpupixel::HealthyFilter::create() {
...
bool HealthyFilter::init() {
  ...
  maskImage = SourceImage::create(Util::getResourcePath("healthy_mask_1.jpg"));
  curveImage = SourceImage::create(Util::getResourcePath("lookup_healthy.png"));
  return true;
}
...
void HealthyFilter::setTexelSize(int textureWidth, int textureHeight) {
  texelSizeX = textureWidth;
  texelSizeY = textureHeight;
}

bool HealthyFilter::proceed(bool bUpdateTargets, int64_t frameTime) {
  ...
  CHECK_GL(glBindTexture(GL_TEXTURE_2D, maskImage->getFramebuffer()->getTexture()))
  _filterProgram->setUniformValue("curve", 3);
  _filterProgram->setUniformValue("mask", 4);
