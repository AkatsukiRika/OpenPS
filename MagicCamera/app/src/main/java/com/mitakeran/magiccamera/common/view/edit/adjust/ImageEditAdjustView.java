package com.mitakeran.magiccamera.common.view.edit.adjust;
...
import com.mitakeran.magiccamera.widget.TwoLineSeekBar;
import com.mitakeran.magiccamera.widget.TwoLineSeekBar.OnSeekChangeListener;
...
public class ImageEditAdjustView extends ImageEditFragment{
	...
	private int convertToProgress(float value){
		...
		} else if (checkedRadioButtonId == R.id.fragment_radio_exposure) {
			exposure = value;
			return (int) Math.round((value + 100) / 2);
		} else if (checkedRadioButtonId == R.id.fragment_radio_saturation) {
			saturation = value;
			return (int) Math.round((value + 100) / 2);
		} else if (checkedRadioButtonId == R.id.fragment_radio_sharpness) {
			sharpness = value;
			return (int) Math.round((value + 100) / 2);
		} else if (checkedRadioButtonId == R.id.fragment_radio_bright) {
			brightness = value;
			return (int) Math.round((value + 100) / 2);
		} else if (checkedRadioButtonId == R.id.fragment_radio_hue) {
			hue = value;
